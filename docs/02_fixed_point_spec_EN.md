...
The final outputs are quantized versions of $Y_0$ and $P$. Hereafter, $P_{\mathrm{re}}$ and $P_{\mathrm{im}}$ denote the wide complex-product results before rounding and saturation.

## 3. Q-Format Convention

Q$m$.$n$ uses $m$ integer-side bits including the sign bit and $n$ fractional bits, for a total width of $m+n$.
...
| Individual $D\times W$ | 33 | Q3.30 | Constrained by actual $D,W$ ranges |
| $P_{\mathrm{re}},P_{\mathrm{im}}$ | 34 | Q4.30 | Add/subtract after explicit 34-bit extension |
...
These are derived values, not independent core state. Y1 flags are evaluated from rounded $Q_{Y_1}$.
