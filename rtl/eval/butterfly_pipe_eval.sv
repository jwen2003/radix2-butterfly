...
// Registered evaluation wrapper for the V1 pipelined butterfly.
//
// This module is used only for synthesis, timing, and physical-design
// comparisons. It adds the same input and output register boundaries used by
// butterfly_comb_eval. The wrapped butterfly core is unchanged.
//
// An input accepted with valid_in=1 on rising edge t produces its corresponding
// evaluation-wrapper output with valid_out=1 on rising edge t+4. The initiation
// interval remains II=1.
module butterfly_pipe_eval (
    input  logic                     clk,
    input  logic                     rst,
